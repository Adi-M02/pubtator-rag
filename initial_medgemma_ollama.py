...
# Uncomment to force a static list during testing
# HARDCODED_DRUGS = [
#     "metformin","atenolol","simvastatin","lisinopril","amlodipine",
#     "omeprazole","furosemide","warfarin","levofloxacin","glyburide",
# ]
...
def ask_llm_for_drugs(n=10) -> List[Dict[str,str]]:
    resp = ollama.chat(
        ...
        messages=[
            ...
            {"role":"user","content":
                f"List exactly {n} distinct prescription-only generic drugs that are commonly prescribed and primarily "
                f"treat diseases with severe implications. For each, include one example disease it treats. "
    ...
    if not txt: raise RuntimeError("LLM returned empty content")
    try:
        data = json.loads(txt)
    except json.JSONDecodeError:
        s = txt.strip(); i,j = s.find("{"), s.rfind("}")
        if i==-1 or j==-1 or j<=i: raise
    ...
    for it in data.get("drugs", []):
        ...
        if d: items.append({"drug": d, "example_disease": ex})
    # dedupe and enforce exact count
    seen, uniq = set(), []
    for obj in items:
        key = json.dumps(obj, sort_keys=True)
        if key not in seen:
            seen.add(key); uniq.append(obj)
    if len(uniq) != n:
...
def main():
    ...
    # Stage 1: LLM
    log.info("Stage 1: asking LLM for 10 drugs")
    drugs_items = ask_llm_for_drugs(10)
    # To use the hardcoded list instead, comment the line above and uncomment below:
    ...
    for de in drug_entities:
        ...
        for cid in de["entity_ids"]:
            rel_map, dis_total = treatment_diseases_for_drug(cid, relation_type="treat", limit=25)
    ...
    # Stage 4
    log.info("\nStage 4: fetching PMIDs per drug–disease (first page, up to 50)")
    for ind in indications:
        cid = ind["drug_id"]
        for did in ind["disease_ids"]:
            results, article_total = search_treatment_evidence(disease_id=did, chemical_id=cid, page=1, page_size=50)
            ...
            log.info(f"  {_pretty(cid)} ~ {_pretty(did)}: {len(pmids)} PMIDs (total={article_total}) {head if head else ''}")
